...
let postBookAppointment = (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.email || !data.doctorId || !data.timeType || !data.date || !data.fullName || !data.selectedGender
                || !data.address) {
                resolve({
                    errCode: 1,
        ...
        try {
            if (!data.doctorId || !data.token) {
                ...
                if (appointment) {
                    ...
                    resolve({
                        errCode: 2,
                        errMessage: 'Appointment confirmed or does not exsit !'
                    })
                }
            }
        } catch (e) {
